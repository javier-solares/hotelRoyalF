...
export const ContentProvider: FC<Props> = ({ children }) => {
  ...
  const all = async () => {
    const response = await GetRoute('TipoPersona/all');
  ...
  const creaetUpdate = async (data: any) => {
    const response = await PostRoute(`TipoPersona/${!data?.id ? 'create' : 'update'}`, {...data, usuario:'81816'});
    all();
    handleClose();
  ...
  const one = async (data: any) => {
    const response = await PostRoute('TipoPersona/one', data);
    setOneData(response.length > 0 ? response[0] : []);
    handleShow();
  };

  const state = async (data: any) => {
    const response = await PostRoute(`TipoPersona/${data?.estado === 1 ? 'destroy' : 'active'}`, data);
    console.log(response.message);
    all();
  };
