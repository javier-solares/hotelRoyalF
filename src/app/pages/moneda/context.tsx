...
export const ContentProvider: FC<Props> = ({ children }) => {
  ...
  const all = async () => {
    const response = await GetRoute('Moneda/all');
  ...
  useEffect(() => {
    const fetchLabelData = async () => {
      try {
        const response = await GetRoute('Moneda/label');
  ...
  const creaetUpdate = async (data: any) => {
    const response = await PostRoute(`Moneda/${!data?.id ? 'create' : 'update'}`, {...data, usuario:'81816'});
    all();
    handleClose();
  ...
  const one = async (data: any) => {
    const response = await PostRoute('Moneda/one', data);
    setOneData(response.data.length ? response.data[0] : []);
    handleShow();
  };

  // const state = async (data: any) => {
  //   const response = await PostRoute(`Moneda/${data?.estado === 1 ? 'destroy' : 'active'}`, data);
  //   console.log(response.message);
  //   all();
  // };
  const Actions = async (data:any) => {
    const response = await PostRoute(`Moneda/status`, {
      id: data.id,
